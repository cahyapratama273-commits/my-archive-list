...
export default async function AnimePage({ searchParams }: PageProps) {
  ...
  const { results, totalPages, totalItems } = getPaginated("anime", currentPage, 24, currentSort);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-900 pb-6 mb-6">
          <div>
            ...
            <div className="text-xs text-zinc-500 mb-2 flex items-center gap-1.5 font-medium">
              ...
              <span>&gt;</span>
              <span className="text-zinc-350">Anime</span>
            </div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight">Daftar Anime</h1>
          ...
          <div className="flex items-center gap-2">
            <Link href="/anime" className="bg-indigo-600 text-white font-bold px-4 py-2.5 rounded-lg text-xs shadow-md shadow-indigo-600/20">
              Anime
            </Link>
            ...
            <Link href="/movie" className="border border-zinc-800/80 bg-zinc-900/40 text-zinc-450 hover:border-zinc-700 hover:text-white px-4 py-2.5 rounded-lg text-xs font-semibold transition-all duration-150">
              Movie
            </Link>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <Link
                  key={opt.value}
                  href={`/anime?sort=${opt.value}&page=1`}
