...
export default async function AnimeDetailPage({ params }: PageProps) {
  ...
  const similar = getSimilar("anime", item.id, 5);
    ...
    <div className="relative min-h-screen bg-zinc-950 px-4 py-10 sm:px-6">
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="text-xs text-zinc-500 mb-6 flex flex-wrap items-center gap-1.5 font-medium">
          ...
          <span>&gt;</span>
          <Link href="/anime" className="hover:text-zinc-350 transition-colors">Anime</Link>
          <span>&gt;</span>
        ...
        <div className="flex flex-col md:flex-row gap-8 sm:gap-10">
          ...
          <div className="flex flex-col items-center md:items-start shrink-0 w-full max-w-[280px] mx-auto md:mx-0">
            ...
            <DetailActions item={item} category="anime" />
          </div>

          {/* Right Column (Info) */}
          <div className="flex-1 flex flex-col gap-5 mt-6 md:mt-0">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-lg bg-pink-950/60 border border-pink-500/30 text-pink-400 px-3 py-1 text-xs font-extrabold tracking-wide uppercase select-none">
                Anime
              </span>
            ...
            <div className="grid grid-cols-2 gap-3.5">
              ...
              <div className="rounded-xl border border-zinc-900 bg-zinc-900/20 p-4 transition-colors hover:border-zinc-850">
                ...
                <span className="text-xs sm:text-sm font-extrabold text-emerald-500">Selesai Tayang</span>
              </div>
              <div className="rounded-xl border border-zinc-900 bg-zinc-900/20 p-4 transition-colors hover:border-zinc-850">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-zinc-500 block mb-1">Sumber</span>
                <span className="text-xs sm:text-sm font-bold text-zinc-200">Manga</span>
              </div>
            </div>
        ...
        {similar.length > 0 && (
          <div className="mt-16 sm:mt-20">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
              {similar.map((simItem) => (
                <Card key={simItem.id} item={simItem} category="anime" />
              ))}
            </div>
          </div>
        )}
